...
enum e_sdk_err_code {
  ...
  e_disparity_not_allowed =
      4, // Disparity or depth image is not allowed to be selected
  e_image_frequency_not_allowed =
      5, // Image frequency must be one of the enum type e_image_data_frequecy
...
typedef struct _image_data {
  ...
  char *m_greyscale_image_left[CAMERA_PAIR_NUM]; /**< greyscale image of left
                                                    camera */
  char *m_greyscale_image_right
      [CAMERA_PAIR_NUM];                /**< greyscale image of right camera */
...
typedef struct _exposure_param {
  float
      m_step; // adjustment step for auto exposure control (AEC). Default is 10.
