...
DJI_lock::DJI_lock() { InitializeCriticalSection(&m_critical_section); }

DJI_lock::~DJI_lock() {}

void DJI_lock::enter() { EnterCriticalSection(&m_critical_section); }

void DJI_lock::leave() { LeaveCriticalSection(&m_critical_section); }

void sleep(int microsecond) { Sleep((microsecond + 999) / 1000); }
DJI_event::DJI_event() {
